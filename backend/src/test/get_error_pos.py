...
try:
    ...
    # Print the bytes before the failure position
    f_in.seek(max(0, pos - 100))
    print("Bytes before:", f_in.read(100))
    # Print the bytes after the failure position
    f_in.seek(pos)
    print("Bytes after:", f_in.read(100))
    traceback.print_exc()
