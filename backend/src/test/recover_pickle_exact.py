import os
import pickle
import sys

model_path = os.path.join(os.getcwd(), 'ml_model', 'churn model')

cp1252_map = {
    '\u20ac': 0x80, '\u201a': 0x82, '\u0192': 0x83, '\u201e': 0x84, '\u2026': 0x85,
    '\u2020': 0x86, '\u2021': 0x87, '\u02c6': 0x88, '\u2030': 0x89, '\u0160': 0x8a,
    '\u2039': 0x8b, '\u0152': 0x8c, '\u017d': 0x8e, '\u2018': 0x91, '\u2019': 0x92,
    '\u201c': 0x93, '\u201d': 0x94, '\u2022': 0x95, '\u2013': 0x96, '\u2014': 0x97,
    '\u02dc': 0x98, '\u2122': 0x99, '\u0161': 0x9a, '\u203a': 0x9b, '\u0153': 0x9c,
    '\u017e': 0x9e, '\u0178': 0x9f,
}

try:
    with open(model_path, 'rb') as f:
        raw_bytes = f.read()
        
    text = raw_bytes.decode('utf-8')
    
    reconstructed_bytes = bytearray()
    for char in text:
        if char in cp1252_map:
            reconstructed_bytes.append(cp1252_map[char])
        else:
            val = ord(char)
            reconstructed_bytes.append(val & 0xFF)
            
    reconstructed_bytes = bytes(reconstructed_bytes)
    
    # Perform the exact reverse newline translations:
    # 1. Map \r\r\n to a temporary token
    # 2. Map remaining \r\n to \n
    # 3. Map the temporary token to \r\n
    original_binary = reconstructed_bytes.replace(b'\r\r\n', b'__TEMP_NEWLINE__')
    original_binary = original_binary.replace(b'\r\n', b'\n')
    original_binary = original_binary.replace(b'__TEMP_NEWLINE__', b'\r\n')
    
    print("Reconstructed bytes len:", len(reconstructed_bytes))
    print("Original binary len:", len(original_binary))
    
    # Try to load the model
    model = pickle.loads(original_binary)
    print("--- Model Loaded Successfully ---")
    print(f"Model Class: {type(model)}")
    if hasattr(model, 'feature_names_in_'):
        print(f"Feature Names ({len(model.feature_names_in_)}): {list(model.feature_names_in_)}")
    if hasattr(model, 'classes_'):
        print(f"Classes: {list(model.classes_)}")
        
    # Save the clean binary model
    clean_model_path = os.path.join(os.getcwd(), 'ml_model', 'churn_model.pkl')
    with open(clean_model_path, 'wb') as f_out:
        f_out.write(original_binary)
    print(f"Clean binary model written to: {clean_model_path}")
    
except Exception as e:
    print(f"Failed to recover model: {e}")
    sys.exit(1)
