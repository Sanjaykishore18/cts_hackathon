...
class DebugSmartUnpickler(pickle.Unpickler):
    def load(self):
        ...
        while True:
            ...
            print(f"Opcode at {pos}: '{opcode_char}' (0x{opcode:02X})")
    ...
    def load_short_binunicode(self):
        ...
        if memoize_idx != -1:
            ...
            print(f"  [SHORT_BINUNICODE] Smart-read: '{string_bytes.decode('utf-8', errors='replace')}' (len={len(string_bytes)}, expected={len_byte})")
        else:
            stream.seek(pos)
            string_bytes = stream.read(len_byte)
            print(f"  [SHORT_BINUNICODE] Default-read: '{string_bytes.decode('utf-8', errors='replace')}' (len={len_byte})")
