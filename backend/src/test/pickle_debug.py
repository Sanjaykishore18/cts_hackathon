    ...
    def load(self):
        ...
        while True:
            ...
            try:
                ...
                print(f"Exception at position {pos} (opcode {opcode_char}): {type(e).__name__}: {e}")
                
                # Print context
                self.file.seek(max(0, pos-20))
                print("Context before:", self.file.read(20))
                print("Failed byte:", bytes([opcode]))
                print("Context after:", self.file.read(20))
                raise e
            if key == b'.':
                break
