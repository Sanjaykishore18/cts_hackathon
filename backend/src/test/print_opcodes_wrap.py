...
class TraceUnpickler(pickle.Unpickler):
    ...
    def log_and_dispatch(self, self_ref, opcode_func, opcode_key):
        # Safely find position from self_ref.read bound method or self_ref._file
        pos = -1
        if hasattr(self_ref, 'read') and hasattr(self_ref.read, '__self__'):
            pos = self_ref.read.__self__.tell() - 1
        elif hasattr(self_ref, '_file'):
            pos = self_ref._file.tell() - 1
